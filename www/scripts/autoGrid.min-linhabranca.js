...
(function (e, f, g) {
    ...
    f.fn.grid = function (j) {
        var h = function (w) {
            ...
            var u = function (aq) {
                if (aq) {
                    al.addClass("grid-loadMore").html("VER MAIS IMAGENS")
                } else {
                    al.removeClass("grid-loadMore").html("")
                }
            };
            var D = function (au, aA, aq) {
                var az = "../../../content/images/linha-branca/";
                if (aq == "no") {
                    az = ""
                }
                var ar = au + "../../../content/images/linha-branca/";
                if (au == "LINHABRANCA") {
                    ar = ""
                }
                var aw = aA.split(/\.(?=[^.]*$)/)[0];
                var av = "<h5>In " + au + "</h5>";
                if (au == "LINHABRANCA" || E.captionCategory == false) {
                    av = ""
                }
                var at = "";
                if (aw.indexOf("$$") != -1) {
            ...
            f.getJSON(window.location.origin+"/reader-linhabranca.php?directory=" + am + "&categoriesOrder=" + E.categoriesOrder + "&imagesOrder=" + E.imagesOrder, function (aq) {
                M = aq;
                C.css("display", "none");
                v();
